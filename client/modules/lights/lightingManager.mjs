...
class LightingManager {
    ...
    initialize(scene) {
        ...
        let sun = new OrbitingLight(scene, 0, new THREE.Vector3(0, 50, 100), 100, [
            ...
            { angle: tau * .25, color: 0xffddaa, intensity: 1.4 },
            { angle: tau * .45, color: 0xfffeea, intensity: 1.6 },
            { angle: tau * .60, color: 0xffcc66, intensity: 1.6 },
            { angle: tau * .75, color: 0xffaa33, intensity: 1.4 },
        ...
        let moon = new OrbitingLight(scene, tau * 0.5, new THREE.Vector3(0, 50, 100), 100, [
            ...
            { angle: tau * .25, color: 0x77aadd, intensity: 0.2 },
            { angle: tau * .4, color: 0x88bbff, intensity: 0.3 },
            { angle: tau * .6, color: 0x88bbff, intensity: 0.3 },
            { angle: tau * .75, color: 0x77aadd, intensity: 0.2 },
        ...
        this.globalLight = new TimeBasedLight(new THREE.HemisphereLight(0x111111, 0xff1111, 0.1), [
            { angle: Math.acos(0.5), color: 0xaabbcc, groundColor: 0xddeeff, intensity: 0.06 },
            { angle: tau * .25, color: 0xdddddd, groundColor: 0xeeffee, intensity: 0.12 },
            { angle: tau * .50, color: 0xdddddd, groundColor: 0xeeffee, intensity: 0.18 },
            { angle: tau * .75, color: 0xdddddd, groundColor: 0xeeffee, intensity: 0.12 },
            { angle: tau - Math.acos(0.5), color: 0xaabbcc, groundColor: 0xddeeff, intensity: 0.06 }
        ]);

        this.spotLight = new TimeBasedLight(new THREE.SpotLight(0xeeeeff, 1, undefined, tau / 50, 1, 0), [
            { angle: tau * .19, color: 0xffffff, intensity: 0.1 },
            { angle: tau * .2, color: 0xffffff, intensity: 0 },
            { angle: tau * .75, color: 0xffffff, intensity: 0 },
            { angle: tau * .76, color: 0xffffff, intensity: 0.1 }
        ]);
